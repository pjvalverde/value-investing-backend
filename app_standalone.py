...
def generate_simulated_historical_data(ticker, period="1year"):
    ...
    # Generar fechas y precios simulados
    end_date = datetime.now()
    start_date = end_date - timedelta(days=days)
    
    # Precio base aleatorio entre 50 y 500
    base_price = random.uniform(50, 500)
    volatility = random.uniform(0.01, 0.05)  # Volatilidad diaria
    ...
    while current_date <= end_date:
        ...
        if current_date.weekday() < 5:  # 0-4 son lunes a viernes
            # Simular movimiento de precio con caminata aleatoria
...
# Variable global para almacenar el último portfolio generado
last_portfolio = {}

# Inicializar servicios
alpha_vantage_client = AlphaVantageClient(api_key=os.getenv("ALPHAVANTAGE_API_KEY"))
...
claude_service = ClaudeService(api_key=os.getenv("CLAUDE_API_KEY"))
...
async def optimize_portfolio(request: Request):
    try:
        ...
        # Guardar el último portfolio generado
...
# Endpoint para análisis de acciones
@app.get("/justification_accion/{ticker}")
async def justification_accion(ticker: str):
    try:
        # Verificar si el ticker existe
        logger.info(f"Obteniendo análisis para {ticker}")
        ...
        try:
            ...
            # Si no se pueden obtener datos reales, generar un análisis simplificado
            simplified_analysis = {
                "ticker": ticker,
                "company_name": ticker,
                "current_price": round(random.uniform(50, 500), 2),
                "change_percent": f"{random.uniform(-5, 5):.2f}%",
                "sector": random.choice(["Tecnología", "Finanzas", "Salud", "Consumo", "Industrial"]),
                "analysis_html": f"<div><h2>Análisis simplificado de {ticker}</h2><p>Este es un análisis simplificado basado en datos simulados.</p></div>",
                "investment_thesis": f"La empresa {ticker} muestra indicadores financieros {random.choice(['positivos', 'mixtos', 'negativos'])}.",
                "recommendation": random.choice(["Comprar", "Mantener", "Vender"])
            }
...
# Función para generar datos históricos simulados
def generate_simulated_historical_data(ticker, period="1year"):
    logger.info(f"Generando datos históricos simulados para {ticker}, periodo {period}")
    
    # Determinar el número de días basado en el periodo
    ...
    for i in range(days, 0, -1):
        ...
        # Agregar algo de ruido para que parezca más real
